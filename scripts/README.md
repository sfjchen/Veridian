...
| `run-teacher-backend.sh` | Start Flask backend on http://localhost:5000 |
...
| Script | Purpose |
|--------|--------|
| `setup-student.sh` | One-time: create venv in `student-platform/`, install deps, set up `.env` files |
...
1. **Teacher:** `./scripts/setup-teacher.sh` then fill in `backend/.env` and `frontend/.env` (see root README).
2. **Student:** `./scripts/setup-student.sh` then fill in `student-platform/.env` and `student-platform/frontend/.env` (see student-platform/README.md).
